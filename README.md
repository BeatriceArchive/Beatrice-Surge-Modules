...
从 v1.6.1 起，任务完成通知会逐项标记登录、观看、分享、投币与大会员大积分签到的结果；对于本轮新完成的项目会明确显示“本次完成”，投币会显示本轮新增枚数及停止时的错误码。同时脚本会在执行前后读取 Bilibili 官方 `/nav` 返回的账号等级经验，并同时展示每日 65 EXP 任务进度的前后变化，用于区分“任务状态已经完成”“经验仍在同步”或“某一写操作实际没有生效”。
