...
Basic Panel 1.4.1 保持单一面板。默认 `YS=1&RISK=1`；省略参数也默认打码并开启信誉查询。`POLICY` 可选，留空跟随 Surge 当前规则；只有 DIRECT 延迟明确走直连。出口、测速及各站点可能分流，面板的“观测出口”不代表所有网站共用此出口。
...
- Disney+、Spotify、TikTok、Prime 和六项 AI 优先用 HEAD 检查网站 / WAF。2xx / 3xx 为“可达”，401 / 403 / 429 / 451 为“受限”，网络超时为“不可达”，其他响应为“未知”；全部不代表账号、地区付费能力或模型功能完整可用。单个站点 403 不改变整体网络颜色。
- **1.4.1 定向复核**：仅 TikTok / Prime 在 HEAD 405 / 501 后执行一次 GET；TikTok 在 HEAD 无状态或传输失败后也复核一次。明确的 401 / 403 / 429 / 451 不触发复核，成功的 HEAD 不重复请求，其他站点仍仅 HEAD。GET 使用原入口 `https://www.tiktok.com/` / `https://www.primevideo.com/`，不跟随跳转、不带 Cookie / Authorization，不查询登录或播放接口。面板附上实际 `GET 200`、`GET 403` 或 `GET 无响应` 等结果；2xx 伴随传输错误不会变成“可达”。国家 / 地区信息不参与这些状态判定。
- 2026-09-11 公开端点探测中，Prime HEAD 为 405、GET 为 307；TikTok HEAD / GET 均为 200。这支持方法复核，但不能代替 iPhone 上的路由 / CDN 验证，也不能据此断言手机端 TikTok 失败的底层原因。流媒体汇总补充非零的“不达 / 未知”，不隐藏失败项。
...
通常无缓存自动刷新发起 **18 个外部请求**：出口及交叉查询 4、延迟 2、流媒体 6、AI 6；归属缓存命中时约 **15 个**。TikTok / Prime 的方法复核按需各增加 1 个，合计最多 **+2**（对应 20 / 17 个），不是每次都 HEAD + GET。另有 2 次本地 HTTP API 调用。Netflix / YouTube 的同站重定向、Netflix 备用样片和出口 fallback 可能增加请求；当前 Profile 流量本地无法读取时，原始托管地址 HEAD → GET 最多再加 2 次，可选 `SUB_URL` 再加 2 次。站点 HEAD 不跟随重定向，也不为了 403 反复尝试。第三方接口各自超时，外部并发低于 Surge 的 20 请求限制。`update-interval=1800` 表示打开策略页时按到期间隔更新，不保证后台每半小时执行。

方法复核 GET 的原生 timeout 为 **4 秒**；回调缺失时脚本 watchdog 在 6 秒内结束等待。不加载页面的图片、脚本等子资源，也不跟随地区跳转。**`discardBody` 只是不保留收到的正文，不等于没有下载正文**；Surge 公开 `$httpClient` API 未提供可配置的小正文硬上限或取消接口。TikTok 此次忽略了 Range，故不依赖 Range 限流，也不声称 GET 只有 1 KiB。复核可能下载完整入口 HTML，仍受 Surge 原生响应限制；未确认更轻量且能代表主站的稳定公开入口，故不以静态图标、robots.txt 或私有接口替代主站证据。
...
6. **1.4.1 定向验收**：分别选择香港、美国节点，点击此面板刷新，记录 TikTok / Prime 完整文字。出现 `(GET …)` 表示新版执行了复核；如 `可达 (GET 200)`、`受限 403 (GET 403)`、`不可达 (GET 无响应)`。Prime 307 也只表示入口可达；如果 GET 仍返回 405 / 5xx，应保留该真实“未知”状态。美国节点仍失败时记录 GET 状态，不按出口国家猜原因；必要时在 Surge 请求列表按两个域名查看 HEAD / GET 的状态及实际策略，无需 MITM。手动刷新仍会执行既有下载估算。
