...
大会员账号会额外执行“大会员经验”签到。这里的“大会员经验”是面板显示名称，底层仍对应 Bilibili 的大积分签到任务。脚本先查询 `/x/vip/vip_center/sign_in/three_days_sign`：当天已经签到则直接跳过；未签到时使用当前成熟实现采用的 `/pgc/activity/score/task/sign2` 完成签到，并在成功后重新查询一次状态。非大会员会安全跳过。大会员签到与普通等级 65 EXP 任务相互独立，并且安排在观看、分享和投币之前执行。
...
投币按“最多 5 枚、扣除今日已投、再受当前余额限制”的规则计算目标。脚本会在每次写入前核对当日投币经验，但对 Bilibili 统计接口的短暂更新延迟采用单调递增处理：远端状态暂时落后时不会把本地已确认成功的投币数回退。每次投币仍只写入 1 枚并在写入前核对当前视频已投数量；v1.6.3 起投币表单补齐成熟 Web 实现使用的 `cross_domain=true`、`eab_x=2`、`ramval=3`、`source=web_normal`、`ga=1` 等字段，同时继续保持 `select_like=0`，不会自动点赞。最多进行有限候选与写入尝试；遇到投币间隔太短等可恢复状态会等待后换候选继续。

从 v1.6.1 起，任务完成通知会逐项标记登录、观看、分享、投币与大会员经验签到的结果；对于本轮新完成的项目会明确显示“本次完成”，投币会显示本轮新增枚数及停止时的错误码。同时脚本会在执行前后读取 Bilibili 官方 `/nav` 返回的账号等级经验，并同时展示每日 65 EXP 任务进度的前后变化，用于区分“任务状态已经完成”“经验仍在同步”或“某一写操作实际没有生效”。
